...
class Dompetitor(Competitor):
    ...
    def process_and_decide(self, rps_state):
        ...
        opponent_move = self.possible_moves.index(opponent_move_str)
    ...
    class Predictor(object):
        ...
        def get_predictor_weight(self,round_weights): 
            assert len(round_weights) == len(self.results), "not enough results from round_weights"
            ...
            for predictor in self.predictors:
                for i in range(min(len(self.weights),len(predictor.results))):
                    if predictor.last_guess is None:
                        continue
                    aggregate_points[predictor.last_guess] += predictor.get_predictor_weight(self.weeights)
            best_moves = []
