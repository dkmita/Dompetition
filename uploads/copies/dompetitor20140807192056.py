...
class Dompetitor(Competitor):
    ...
    class LastPredictor(Predictor):
        ...
        def get_prediction(self):
            if self.dom_last_move is None:
                self.last_guess = None
                return 0
            self.last_guess = (self.dom_last_move + 1) % len(self.possible_moves)
            return self.last_guess

        def get_predictor_weight(self,round_weights):
            return 10*super(self.__class__, self).get_predictor_weight(round_weights)
    ...
    def __init__(self):
        ...
        self.dom_last_move = None
        self.agg_predictor = self.AggregatePredictor([6,4,3,2,1.5,1.25,1,0.75,0.5,0.5])
        for pattern in [[0],[1],[2],[0,1],[0,2],[1,0],[1,2],[2,0],[2,1],\
                        [0,0,1],[0,0,2],[0,1,0],[0,1,1],[0,1,2],[0,2,2],\
                        [1,0,0],[1,0,1],[1,0,2],[1,1,0],[1,1,2],[1,2,2],\
