    ...
    def get_prediction(self):
        ...
        for predictor in reversed(self.predictors):
            predictor_scores += [(sum(predictor.results),-len(predictor.move_order), \
                                 predictor.get_prediction(), predictor.move_order)]
        predictor_scores.sort()
        prediction = predictor_scores[-1][2]
        print prediction, predictor_scores[-1][3]
        self.last_guess = prediction
        return prediction
...
class SimpleDomp: 
    ...
    def __init__(self): 
        ...
        self.dom_last_move = None 
        self.agg_predictor = BestAggregatePredictor() 
