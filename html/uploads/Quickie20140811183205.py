...
class Quickie(Competitor):
    ...
    def __init__(self):
        ...
        self.moves_dict = {'S':'R', 'R':'P', 'P':'S'} # move : what beats that move
        pass
    ...
    def process_and_decide(self, state):
        ...
        if len(self.opponents_moves) >= 3:
            if (self.opponents_moves[-1] == self.opponents_moves[-2] and
                ...
                return (self.moves_dict[opponents_move], 'how you like them apples?')
            
        return (random.choice(self.moves_dict.keys()), 'how you like them apples?')
