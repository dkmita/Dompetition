...
class Strategy(object):
    ...
    def predict_opp_move(self, my_moves, opp_moves):
        weight = 0.9
        num_r = sum([weight**(len(opp_moves)-x) for x in enumerate(opp_moves) if opp_moves[x] == "R"])
        num_p = sum([weight**(len(opp_moves)-x) for x in enumerate(opp_moves) if opp_moves[x] == "P"])
        num_s = sum([weight**(len(opp_moves)-x) for x in enumerate(opp_moves) if opp_moves[x] == "S"])
